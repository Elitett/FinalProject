...
CREATE TABLE grades(
studentID int NOT NULL REFERENCES students(sid), -- foreign key 
teacherID int NOT NULL REFERENCES teachers(tid), -- foreign key 
subjectID int NOT NULL REFERENCES subjects(subid), -- foreign key 
grade int NOT NULL,
...
-- List all student names that have not taken any classes (don't have grade in grades table)
SELECT name FROM students WHERE sid NOT IN (SELECT studentID FROM grades);

-- List all student names that have taken at least one class
